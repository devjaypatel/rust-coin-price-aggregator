...
pub async fn start_price_agg() -> Result<(), std::io::Error> {
    ...
    HttpServer::new(move || {
        App::new()
            ...
            // .service(web::resource("/add-task").route(web::get().to(api::add_tasks)))
            // .service(web::resource("/task/{id}")))
            .service(
                web::resource("/task/{id}"),
                // .route(web::get().to(api::get_task))
                // .route(web::delete().to(api::delete_tasks))
                // .route(web::put().to(api::update_task))
            )
    })
    .bind(("127.0.0.1", 8080))
